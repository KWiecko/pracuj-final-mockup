...
# defining local path for sourcing codes

pathToFuns <- "scripts/harvester/"


# loading best prediciotn model -> varname: myBestTRee
get(load(paste0(pathToFuns, "bestRFKappa.Rdata")))
...
# reading needed dictionaries:

source(paste0(pathToFuns, "readNeededTables.R"))
listOfTables <- readNeededTables(polaczenie)
...
for (i in 1:nrow(idLinks)) {
  ...
  #scoring offer
  source(paste0(pathToFuns, "extractSkillsTable.R"))
  # extractSkillsTable returns vector of:
  # [ (0/1 <=> isDS/notDS) value , [%] of trees voting for 'isDS' classification ]
  predictionCols <- extractSkillsTable(id,
                                       description %>% as.data.frame(),
                                       position %>% as.data.frame(),
                                       href %>% as.data.frame(),
                                       listOfTables,
                                       myBestTRee)
  ...
  dbGetQuery(
    polaczenie,
    paste0(
      "INSERT INTO offers (id, href, position, date, location, grade, employer, description, main_category, sub_category, salary, contract, is_ds_offer, model_score) VALUES ('",
      id,"','",
      href,"','",
      position,"','",
      date,"','",
